...
case class Task(id: Long, description: String, complete: Boolean) {

  def save()(implicit session: DBSession = Task.autoSession): Task =
    Task.save(this)(session)

  def destroy()(implicit session: DBSession = Task.autoSession): Unit =
...
object Task extends SQLSyntaxSupport[Task] {
  ...
  def apply(t: SyntaxProvider[Task])(rs: WrappedResultSet): Task =
  ...
  def create(description: String, complete: Boolean)(
    ...
    Task(id = generatedKey, description = description, complete = complete)
  }
